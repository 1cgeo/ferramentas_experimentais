...
class ElevationPointsGeneralization(QgsProcessingAlgorithm): 
    ...
    def processAlgorithm(self, parameters, context, feedback):      
        ...
        pointLayerpre = self.parameterAsVectorLayer( parameters,'INPUT_ELEVATION_POINTS', context )
        ...
        pointLayer = self.runAddCount(pointLayerpre)
        self.runCreateSpatialIndex(pointLayer)
        ...
        CRSstr = pointLayerpre.sourceCrs()
        pointsSelected = []
        if (gridScaleParam==0):
            gridScale = 25000
        ...
        self.generalizePoint(grid, pointLayer, pointsSelected, summitsOrBottoms, isDepressionField)
        self.outLayer(parameters, context, pointsSelected, CRSstr)
    ...
    def generalizePoint(self, grid, pointLayer, pointsSelected, summitsOrBottoms, isDepressionField):
        isDep = 1
        isNotDep = 0  
        for g in grid.getFeatures():
            request = QgsFeatureRequest().setFilterRect( g.geometry().boundingBox() )  
            pointsSelectedinGrid=[]
            ...
            for point in pointLayer.getFeatures(request):
                for SoB in summitsOrBottoms:
                    if (point.geometry().within(SoB.geometry())):
                        ...
                        break
                if (not hasSoBPoints):
                    if (not pointsSelectedinGrid):
                        pointsSelectedinGrid.append(point)
                        continue
                    if (pointsSelectedinGrid[-1]['cota']>point['cota']):
                        pointsSelectedinGrid.append(point)
            if (pointsSelectedinGrid):
                if (pointsSelectedinGrid[-1] not in pointsSelected):
                    pointsSelected.append(pointsSelectedinGrid[-1])
